    ...
    return(
        <section className={`section w-full h-full flex justify-center items-center lg:items-start flex-col lg:flex-row gap-10 grow ${(title === 'Minimalismo' || title === 'Maximalismo') ? 'gap-10' : ' gap-15 sm:gap-20'}`}>
            <div className="section__content  flex flex-col items-end gap-15 relative w-full sm:w-fit">
                ...
                <header data-swiper-parallax="-300" className={`section__header p-5 sm:p-10 self-start pl-20 sm:pl-30 sm:mr-30 lg:mr-40 relative w-fit ${color}`}>
                    <img data-swiper-parallax="-200" className="section__banner absolute top-10 -left-[40%] sm:-left-[50%] w-[70%] sm:w-[80%] max-w-72" src={banner} alt="" />
                ...
                <article className="article flex flex-col gap-5 ml-22">
                    <p data-swiper-parallax="-400" className="article__text text-sm w-full sm:text-lg max-w-60 sm:max-w-80 ">{text}.</p>
